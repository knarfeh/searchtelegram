...
# build pkgs
RUN apk --update --no-cache add openssl postgresql-dev gcc g++ python3-dev musl-dev make curl
...
COPY ./e2e /src/
COPY e2e/conf/default.conf /etc/nginx/
COPY e2e/conf/crontab /var/spool/cron/crontabs/root
COPY e2e/conf/supervisord.conf /etc/supervisord.conf
